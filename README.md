...
- **The command line** (`cli`): `avelorn units list` and `avelorn units show <slug>`, the same pair for `weapons`, `armour` and `rules`, and `avelorn rules list --unmodelled` for every printed rule the engine does not yet apply.
- **The HTTP surface** (`api`): list and `/{slug}` reads for units, weapons, armour and rules, plus `POST /muster` (size, equip and cost one block), `POST /fight` (one round of close combat, exact) and `POST /volley` (a volley and the panic it causes). Responses are the schema types, so the OpenAPI document at `/docs` comes from the same models the YAML validates against. `make serve` runs it.
...
- **The front end** (`frontend`) is where it comes together. Build an army list; `POST /muster` costs each block. Put blocks on the **table**: drag to move, rotate, drag a side edge to re-form, drop one block on another to resolve a charge, a shot or a melee, and read the result back as full distributions drawn as charts. Every printed name opens as a pane: datasheet, rule, weapon, armour. List legality is not checked yet.
...
Requires Python 3.12+ and [uv](https://docs.astral.sh/uv/); the front end also needs Node.
...
make frontend  # serve the browser at http://localhost:5173 (with make serve in another shell)
make up        # or bring both up in containers instead
```

The battle table is at [localhost:5173/table](http://localhost:5173/table) once both are up.
