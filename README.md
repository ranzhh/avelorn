...
- **The front end** (`frontend`) is a SvelteKit browser over that HTTP surface: the datasheet list, filterable, and one datasheet in full. Its TypeScript types are generated from the API's OpenAPI document (`make types`), so the Pydantic schema stays the only place a unit's shape is written down.
