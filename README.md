...
- **The front end** (`frontend`) is a SvelteKit browser over that HTTP surface: the datasheet list, filterable, one datasheet in full, and the rule entries its special rules link to -- a rule the engine does not model is marked as such rather than linked. It also builds an army list: pick a datasheet, size it, tick the options it offers, and `POST /muster` costs the block. A block stays editable -- reopen it to change the model count or the wargear and it is recosted, or duplicate it to take the same unit again -- and an open block shows the loadout it actually ends up with, the chosen options folded in. The **table** page is where the maths surfaces. A unit browser down the left, grouped by the army that fields it, deploys a block straight onto a battle view: the rectangle drawn is what the block actually occupies, its files and ranks on the datasheet's bases. Pick one block up, click another, and the menu that opens offers what it could do -- charge, shoot if it carries anything that can, or fight a melee it is already in. The separation and the arc struck are measured off the table rather than typed, which is the point of drawing it. What comes back is not an average: the whole casualty distribution each way, who wins the round, what the loser's Break test or the target's nerve does, and every rule the engine held without applying. Blocks face one of four quarters and are wheeled a quarter at a time; dragging them, and any notion of a turn, are not modelled -- a result is read, never applied back onto the table. Nothing checks whether a list is *legal* — army composition is not modelled yet — so it totals blocks and refuses only what a datasheet itself forbids. The list and the table both live in the browser, since there is no store behind the API to keep them in. Its TypeScript types are generated from the API's OpenAPI document (`make types`), so the Pydantic schema stays the only place a unit's shape is written down.
