...
- **The HTTP surface** (`api`) is the same window over the wire: `GET /units`, `GET /units/{slug}`, `GET /rules`, `GET /rules/{slug}` and `GET /rules/unmodelled`, plus `POST /muster`, which sizes and equips one datasheet and hands back what the block costs, and `POST /fight`, which puts two of them in close combat and resolves a round exactly. The responses are the schema types themselves, so the OpenAPI document at `/docs` is generated from the same models the YAML validates against. `make serve` runs it. Both surfaces show a unit through one declared view (`tow/views`), so neither can fall behind the other.
