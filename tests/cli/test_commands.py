...
def test_units_lists_every_datasheet() -> None:
    ...
    lines = commands.units(GAME)
    assert len(lines) == len(GAME.units) + 1
    assert all(slug in "\n".join(lines) for slug in GAME.units)


def test_show_prints_every_profile_row() -> None:
    """A datasheet with a champion row prints both rows, under the characteristics."""
    printed = "\n".join(commands.show(GAME, "white-lions-of-chrace"))
