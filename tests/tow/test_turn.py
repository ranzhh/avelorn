...
def test_a_charge_registers_its_engagement_on_the_turn() -> None:
    """A Movement-phase charge leaves its engagement in turn.engagements."""
    turn = GAME.turn()
    with turn.movement() as mv:
        engagement = mv.charge(_spearmen(10), _archers(10), Charge(8, ChargeArc.FRONT))
    assert turn.engagements == [engagement]


def test_the_combat_phase_fights_the_turns_engagements() -> None:
    """Combat fights the engagement the charge formed; the charger strikes first."""
    turn = GAME.turn()
    with turn.movement() as mv:
        engagement = mv.charge(_spearmen(10), _archers(10), Charge(8, ChargeArc.FRONT))
    ...
    with turn.combat() as cb:
        fought = [
            cb.fight(
                e, a_weapon=REPO.weapons["thrusting-spear"], b_weapon=REPO.weapons["hand-weapon"]
            )
            for e in turn.engagements
        ]
    assert len(fought) == 1
    assert fought[0].first_striker is engagement.a  # the charger struck first
