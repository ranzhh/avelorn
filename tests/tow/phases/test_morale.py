...
def _spearmen(*rules: Rule) -> Contingent:
    # A Ld 8 defender carrying exactly the given resolved rules. The
...
def test_reroll_effect_lifts_the_pass_probability() -> None:
    ...
    The defender's loadout carries Valour of Ages resolved, so the
...
def test_valour_of_ages_applies_from_the_data_file() -> None:
    """End to end: fielding resolves the authored effect, and it re-rolls."""
