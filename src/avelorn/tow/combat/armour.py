...
# A positional tuple return, and a registry threaded in by the caller,
# degrade as more such resolvers accrue (rules, weapons, ...). It is
...
def defender_armour(defender: Unit, armoury: Registry[Armour]) -> tuple[int | None, list[str]]:
    """Resolve a defending unit's armour value from its worn equipment.

    ``armoury`` resolves printed equipment names to armour items;
    equipment it cannot resolve is reported in the returned notes rather
